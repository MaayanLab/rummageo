...
export default function TermTable({
  ...
  const dataFiltered = React.useMemo(
    () =>
      terms.filter((el) => {
        return (
          el?.title?.toLowerCase() + (el?.geneSetById?.species ?? "")
  ...
  return (
    <>
      <GeneSetModal
        geneset={genesQuery?.data?.geneSet?.genes.nodes.map(
          ({ symbol }) => symbol
        )}
        term={currTerm}
        showModal={showModal}
      ...
      <SamplesModal
        ...
        setShowModal={setShowConditionsModal}
      ></SamplesModal>
                ...
                const blob = blobTsv(
                  ...
                  (item) => {
                    ...
                    return {
                      ...
                      title: item?.title ?? "",
                      condition1Title:
                        item?.sampleGroups["titles"][cond1] ?? "",
                      condition2Title:
                        item?.sampleGroups["titles"][cond2] ?? "",
                      condition1Samples:
                        item?.sampleGroups["samples"][cond1] ?? "",
                      condition2Samples:
                        item?.sampleGroups["samples"][cond2] ?? "",
                      direction: dir,
                ...
                return (
                  <tr key={el?.term}>
                    ...
                    <td>
                      ...
                      >
                        {cond1Title ? cond1Title.split(filterTerm).map((part: string, i: number) => (
                          <>
                            {i > 0 && <b>{filterTerm}</b>}
                            {part}
                          </>
                    ...
                    <td>
                      ...
                      >
                        {cond2Title ? cond2Title.split(filterTerm).map((part: string, i: number) => (
                          <>
                            {i > 0 && <b>{filterTerm}</b>}
                            {part}
                          </>
